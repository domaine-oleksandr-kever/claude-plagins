...
1. **Ingest the Jira ticket(s).** **Context-first:** if the conversation context already contains *all* of the fields this skill needs (Description, AC, **Technical Approach**, **Steps to Test**, links) in full (not summarized or truncated — e.g. from an earlier skill run or a pasted ticket), use that and **skip the fetch**. Second stop: the **task workspace** — `.claude/fnd/<TICKET>/` (or `.claude/fnd/<branch-slug>/` with `ticket-<KEY>.md` files, when several tickets ship in this PR) may already hold the ticket(s), per-bug root causes and preview-theme breadcrumbs (`notes.md`), and progress from an earlier skill or session; if fresh (rules: `${CLAUDE_PLUGIN_ROOT}/references/task-workspace.md`), use it and skip the fetch too. Otherwise **delegate to the `jira-reader` subagent** (pass the ticket key/URL): it reads via Atlassian MCP, applies `${CLAUDE_PLUGIN_ROOT}/references/jira-custom-fields.md`, and returns the structured fields, keeping the raw ADF out of this context. **If the PR covers several tickets, spawn one `jira-reader` per key in parallel** and merge their fields (the body lists all tickets; the title carries all keys). If any returns `needs_clarification`, ask the developer. **After any fresh fetch, save the reader output(s) to the task workspace** so later skills and sessions skip the refetch.
...
**Mark progress:** after sharing the PR URL, check off this workflow's row in the ticket's workspace `progress.md` (creating the folder/file if absent — `${CLAUDE_PLUGIN_ROOT}/references/task-workspace.md`) with date + the PR URL. Then **offer the next unchecked step** in one line and wait; never auto-run it: `/fnd:write-steps-to-test <ticket>` when the ticket's Steps to Test field is still empty; otherwise the series is complete.
