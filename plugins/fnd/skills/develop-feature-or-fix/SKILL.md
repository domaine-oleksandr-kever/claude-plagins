...
5. **Read every linked doc.** Once `jira-reader` returns the links, ingest them all per `${CLAUDE_PLUGIN_ROOT}/references/reading-linked-docs.md` (reuse-before-fetch, per-type tools, save extracts to the workspace; Figma is already covered by step 4). **Notion links are mandatory — if the Notion MCP isn't connected, stop and tell the developer** which links you couldn't read (enable `/mcp` or paste the content); don't plan around them. Extract only what the task needs — especially any **data-mapping / schema** that defines metafields or metaobjects (feeds step 6).
